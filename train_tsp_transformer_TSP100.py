...
with torch.autograd.profiler.emit_nvtx():
    ...
    for epoch in range(0,args.nb_epochs):
        ...
        for step in tqdm(range(1, args.nb_batch_per_epoch + 1), ascii=True):

            # generate a batch of random TSP instances    
            nvtx.range_push("Create tsp instances")
            ...
            nvtx.range_push("Compute tours for model")
            ...
            nvtx.range_pop()
            nvtx.range_push("Update parameters")
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            nvtx.range_pop() # End Update params
